...
public class VistaRepositoryImpl implements VistaRepository {
    ...
    @Resource(type = ClinicalReminderRepository.class)
    ...
    private void filterClinicalReminders(
        ...
        Map<String, ClinicalReminder> allowedCrMap = clinicalReminderRepository.deriveAllowedClinicalReminders();
        for (Iterator<VistaVeteranClinicalReminder> iter = resultList.iterator(); iter.hasNext(); ) {
            ...
            if (allowedCrMap.get(vvcr.getClinicalReminderIen()) == null) {
                iter.remove();
            }
        }
    }
