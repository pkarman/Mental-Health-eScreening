            ...
            aoColumns = [
                ...
                {"mData": "comment", "sWidth": "150px", "sClass": "wrap", "bSortable": false},
                {
                    ...
                    "mRender": function (data, type, full) {
                        return '<a href="#"  data-toggle="modal" data-target="#modal_confirmation" onclick="' + checkDataType(full.exportType, full.exportLogId) + '" ng-model="exportIdentifiedDataButton" data-exportLogId="' + full.exportLogId + '"  data-exportType="exportAgain" class="font-size-24"><span class="glyphicon glyphicon-download"></span></a>';
                    }
                }];
...
function exportDataController($scope, $element, $http, $window, programListService, filterOptionService) {
    ...
    $scope.launchDataDictionaryButton = function () {
        ...
        $http({method: 'GET', url: 'exportData/services/exports/genDataDictionary'}).
    ...
    $scope.searchDatabase = function () {
        ...
        if (typeof $scope.exportDataFormBean.exportLogId === "undefined") {
            ...
            $http({method: 'GET', url: 'exportData/services/exports/genExportData?' + $scope.qstring}).
                success(function (data, status, headers, config) {

                    window.open("exportData/services/exports/getExportDataFromSession", "_self");
                    $('#modal_progress').modal('hide');
                }).
                error(function (data, status, headers, config) {
                    $("#progressMessages").html("<div class='alert alert-danger' role='alert'>Error occurred:" + data + "</div>");
                });
    ...
    $scope.getDataForSearch = function (sSource, aoData, fnCallback, oSettings) {
        ...
        aoData.push({
            "name": "exportDataFormBean.veteranAssessmentId",
            "value": $scope.exportDataFormBean.veteranAssessmentId
        });
        ...
        aoData.push({
            "name": "exportDataFormBean.showDeletedClinicians",
            "value": $scope.exportDataFormBean.showDeletedClinicians
        });
        aoData.push({"name": "exportDataFormBean.createdByUserId", "value": $scope.exportDataFormBean.createdByUserId});
        aoData.push({
            "name": "exportDataFormBean.showDeletedAssessmentCreators",
            "value": $scope.exportDataFormBean.showDeletedAssessmentCreators
        });
        aoData.push({
            "name": "exportDataFormBean.fromAssessmentDate",
            "value": $scope.exportDataFormBean.fromAssessmentDate
        });
        aoData.push({
            "name": "exportDataFormBean.toAssessmentDate",
            "value": $scope.exportDataFormBean.toAssessmentDate
        });
